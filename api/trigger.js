// api/trigger.js — receives blobUrl + filename, triggers Kaggle kernel
...
export default async function handler(req, res) {
  ...
  if (!kaggleUsername || !kaggleKey) {
    return res.status(500).json({ error: 'Kaggle credentials not configured.' });
  }
  ...
  try {
    ...
    console.log(`[trigger] Kaggle push response:`, JSON.stringify(data));
    if (!kaggleRes.ok) throw new Error(data.message || `Kaggle API ${kaggleRes.status}`);
    console.log(`[trigger] Job ${jobId} — Kaggle kernel queued`);
    return res.status(200).json({ ok: true, jobId });
  } catch (err) {
    console.error('[trigger] Kaggle error:', err.message);
